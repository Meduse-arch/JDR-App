...
export function useCompetenceUsage(
        ...
        for (const m of comp.modificateurs) {
          if (m.type_calcul === 'roll_dice' || m.type_calcul === 'roll_stat') {
            ...
            if (m.type_calcul === 'roll_dice') {
              ...
              const finalValeurStat = Math.round((valBase + (bonusFixesStats[m.des_stat_id!] || 0)) * (1 + (bonusPctStats[m.des_stat_id!] || 0) / 100));
  ...
  }, [personnage, afficherToast, mettreAJourLocalement, setDiceResult, statsCalculees, roleEffectif, sessionActive, pnjControle]);
