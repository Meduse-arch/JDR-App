...
const getDestiny = (roll: number, faces: number, isTotal: boolean = false, nb: number = 1) => {
  ...
  const pct = Math.round((roll / (faces * nb)) * 100);
...
const DiceShape = ({ faces, className }: { faces: number, className?: string }) => {
  ...
  switch (faces) {
    ...
    case 10:
      return (
        <svg viewBox="0 0 100 100" {...props}>
          <path d="M50 10 L85 40 L72 85 L28 85 L15 40 Z" />
        </svg>
      );
    case 12:
      return (
        <svg viewBox="0 0 100 100" {...props}>
          <path d="M50 10 L80 25 L90 55 L70 85 L30 85 L10 55 L20 25 Z" />
        </svg>
      );
    case 20:
      return (
        <svg viewBox="0 0 100 100" {...props}>
          <path d="M50 10 L85 28 L85 72 L50 90 L15 72 L15 28 Z" />
        </svg>
      );
    case 100:
    default:
...
const getAbbreviatedLabel = (label: string) => {
  return STAT_ABBR[label] ?? label.substring(0, 3).toUpperCase();
};

export const DiceRollModal: React.FC = () => {
  const { diceResult, setDiceResult, sessionActive } = useStore();
  const [phase, setPhase] = useState<'rolling' | 'rune' | 'reveal'>('rolling');
  const [displayRune, setDisplayRune] = useState('');

  // Listener pour les lancers de dés des autres joueurs
  useEffect(() => {
    if (!sessionActive?.id) return;

    const unsubscribe = broadcastService.subscribe(sessionActive.id, 'dice-roll', (payload) => {
      // Rétrocompatibilité au cas où l'ancien format (array direct) circule encore
      if (Array.isArray(payload)) {
        setDiceResult(payload, false);
        return;
      }
      ...
      if (isSecret) {
        ...
        if (myId && myId !== senderId) {
          // On vérifie d'abord les viewers envoyés par le lanceur
          let isAllowed = Array.isArray(allowedViewers) && allowedViewers.includes(myId);

          // S'il n'est pas autorisé via le lanceur, on vérifie notre propre configuration locale
          // (utile si c'est le MJ qui regarde le jet d'un joueur qui n'a pas la config à jour)
          if (!isAllowed) {
            const partagesDes = state.sessionActive?.parametres?.partagesDes || {};
            const localAllowedViewers = partagesDes[senderId] || [];
            if (localAllowedViewers.includes(myId)) {
              isAllowed = true;
            }
          }

          if (!isAllowed) {
            // Non autorisé à voir ce jet secret
            return;
          }
        }
      }

      setDiceResult(diceResult, false); // false = ne pas re-broadcaster
    });
  ...
  const rollInfo = useMemo(() => {
    ...
    diceResult.forEach((res: DiceResult) => {
      ...
      const faces = match ? parseInt(match[3] || match[2]) : 20;
    ...
    const pct = Math.round((totalRoll / totalMaxPossible) * 100);
    if (pct > 80) return { char: 'ᛟ', label: 'Triomphe' };
    if (pct > 60) return { char: 'ᚨ', label: 'Faveur' };
    if (pct > 40) return { char: 'ᛁ', label: 'Équilibre' };
    if (pct > 20) return { char: 'ᚱ', label: 'Résistance' };
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setDisplayRune(RUNES[Math.floor(Math.random() * RUNES.length)]);
    }, 40);
  ...
  return (
    <ModalContainer onClose={() => setDiceResult(null)} className="items-center">
      <div className="flex flex-col items-center gap-3 py-4">
        {/* Rune — visible en phases rolling ET rune, disparaît en reveal mais reste en petit au-dessus */}
        {phase !== 'reveal' ? (
          <span className={`font-serif leading-none transition-all duration-300 ${
            phase === 'rune'
              ...
              : 'text-5xl text-theme-main/60'
          }`}>
            {displayRune}
          </span>
        ) : (
          <div className="flex flex-col items-center gap-1 animate-in fade-in duration-300">
            {/* Rune finale en petit avec glow pulsé */}
            <span className="text-2xl font-serif text-theme-main/70 animate-pulse drop-shadow-[0_0_10px_rgba(var(--color-main-rgb),0.6)]">
              {rollInfo.char}
            </span>
            {/* Chiffre total avec forme du dé en filigrane */}
            <div className="relative flex items-center justify-center">
              {/* Filigrane forme du dé derrière */}
              {diceResult.length === 1 && (() => {
                const match = diceResult[0].diceString.match(/(\d+)d\(?([^=)]+)=?(\d+)?\)?/)
                const faces = match ? parseInt(match[3] || match[2]) : 20
                return <DiceShape faces={faces} className="absolute w-24 h-24 opacity-[0.06]" />
              })()}
              <span className="relative text-7xl font-cinzel font-black text-theme-main drop-shadow-[0_0_15px_rgba(var(--color-main-rgb),0.3)] leading-none z-10">
                {totalGlobal}
              </span>
            </div>
            {/* Nom du destin */}
      ...
      {phase === 'reveal' && (
        <div className="animate-in fade-in duration-500 w-full flex flex-col gap-6">
            ...
            {diceResult.map((res: DiceResult, i: number) => {
              ...
              return res.rolls.map((val: number, j: number) => (
                <div key={`${i}-${j}`} className="flex flex-col items-center gap-1 min-w-[30px]">
                  ...
                  <span className="font-cinzel text-[8px] text-theme-main/40 uppercase text-center">{label}</span>
          ...
          <div className="flex flex-wrap justify-center gap-2 opacity-60 w-full pt-4 border-t border-theme/10">
            {diceResult.map((res: DiceResult, i: number) => (
              <div key={i} className="text-[9px] font-mono border border-theme-main/20 px-2 py-0.5 text-theme-main bg-theme-main/5 uppercase">
